...
channels that are not considered by TGP. Independent numerical
verification via one-loop exact renormalization group (ERG) gives a
fixed-point value indistinguishable from $2$ to the precision of the
calculation (row~16 of Sec.~\ref{sec:status}).
\end{remark}
...
21 & Coarse-graining $H_{\Gamma}\!\to\!S_{\mathrm{TGP}}$ (numerical)
  & Block-spin Monte Carlo and FRG LPA' on a 1D substrate chain show
    $K_{\mathrm{IR}}/K_{\mathrm{UV}}=1.000$ at the $\alpha=2$ fixed point
    (8/8 PASS, \texttt{research/continuum\_limit/}): numerical support for
    OP-6; rigorous proof open.
  & \textsf{NU}\\
\bottomrule
\end{longtable}
\end{center}
