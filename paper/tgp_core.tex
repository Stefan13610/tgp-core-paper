...
$c_{\mathrm{GW}}=c_{0}$ in the scalar sector\footnote{The full
GW170817 consistency, including the dispersionless propagation of
the tensor (TT) polarisations, is conditional on the closure of OP-7
T3--T6 (dynamics of the composite $\sigma_{ab}$ projection from the
substrate kinetic term, kinematics validated 2026-04-25; see
Cor.~\ref{cor:cGW} and \texttt{KNOWN\_ISSUES.md} entry C4).},
...
OP-7 & Tensor projection $\sigma_{ab}$
  ...
  \textbf{Kinematic half closed (2026-04-25):} T1 (no-tensor for single-$\Phi$
  M9.1$''$ ansatz, only breathing mode), T2 ($\sigma_{ab} = K_{ab} - \tfrac{1}{3}\delta_{ab}\,\mathrm{Tr}(K)$
  with $K_{ab} = \langle (\partial_a \hat s)(\partial_b \hat s) \rangle_B$,
  validated 12/12 PASS as gradient-strain composite from $H_{\Gamma}$ kinetic
  term). \textbf{Dynamical half (T3-T6) open:} $\sigma_{ab}$ EOM, mass $m_\sigma$,
  metric coupling $\Lambda(\psi)$, quadrupole formula, full PPN consistency.
  See \texttt{research/op7/OP7\_T\{1,2\}\_results.md} and
