...
OP-9 & Strong-field black-hole solutions
  ...
  (CLOSED CONDITIONAL POSITIVE, 2026-04-25, 13/18 = 72\% PASS):}
  T1 PN truncation 4/4 ROBUST GENUINE; T2 mass calibration MC 1/3
  (Sgr A$^*$ DISCRIMINATOR, 4$\sigma$); T3 q-renormalization 4/5
  ...
  See \texttt{research/op-eht/OP\_EHT\_final\_verdict.md} and
  \texttt{KNOWN\_ISSUES.md}.
  & \textsf{HY}\\
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Black-hole shadows.} In the strong-field regime the
    ...
    with first-principles derivation from the M9.1$''$ Lagrangian as the
    open path forward. Astrophysical environment (accretion + plasma)
    saves M87$^*$ but cannot bridge the Sgr A$^*$ gap (T4: 1/3 PASS).
    The ngEHT 2030--2032 timeline at $\sim 1\%$ shadow precision delivers
    $\geq 10\sigma$ verdict on TGP M9.1$''$ via Sgr A$^*$ (T5: 3/3 PASS).
    The tensor sector is independently closed via OP-7 (94/97 = 96.9\% PASS,
    ...
    under~\eqref{eq:exp-metric} is captured by OP-EHT, with the
    \texttt{research/op-eht/} directory containing T1--T5 numerical
    audits and \texttt{OP\_EHT\_final\_verdict.md} the synthesis.
